...
class TradingEnvironment:
    def __init__(self, config):
        ...
        self.generation_length = env_config[
            "generation_length"
    ...
    def _collect_trade_requests(self) -> List[Tuple]:
        ...
        for agent in self.agents:
            ...
            trade_action = agent.select_trade_action(
                self.market_data, self.agent_positions
            )

            logger.debug(f"Agent {agent.agent_id} trade action: {trade_action}")

            if trade_action is not None:
                target_agent_id, item_giving, item_wanting, amount_wanting = (
                    trade_action
                )

                # Validate trade request
                if self._validate_trade_request(
                    agent, target_agent_id, item_giving, item_wanting, amount_wanting
                ):
                    ...
                    logger.debug(
                        f"Valid trade request: {agent.agent_id} -> {target_agent_id}, {item_giving} for {item_wanting}"
                    )
                else:
                    logger.debug(
                        f"Invalid trade request from {agent.agent_id}: insufficient resources"
                    )
            else:
                logger.debug(f"Agent {agent.agent_id} selected no trade action")

        logger.info(
            f"Collected {len(trade_requests)} valid trade requests from {len(self.agents)} agents"
        )
    ...
    def _select_trade_by_distance(
        self, target_id: str, requests: List[Tuple]
    ) -> Optional[Tuple]:
        """
        ...
        for request in requests:
            ...
            if requester_id in self.agent_positions:
                ...
                distance = np.linalg.norm(
                    np.array(target_pos) - np.array(requester_pos)
                )
    ...
    def _try_execute_trade(self, request: Tuple) -> Optional[Dict]:
        ...
        if not self._validate_trade_request(
            requester, target_id, item_giving, item_wanting, amount_wanting
        ):
            return None
        ...
        actual_giving_amount = min(
            required_giving_amount, requester.inventory[item_giving]
        )
        ...
        if (
            actual_giving_amount <= 0
            or actual_wanting_amount <= 0
            or actual_giving_amount < min_trade_threshold
            or actual_wanting_amount < min_trade_threshold
        ):
            return None
        ...
        logger.debug(
            f"Trade executed: {requester_id} gave {actual_giving_amount:.2f} {item_giving} "
    ...
    def _end_generation(self):
        ...
        fitness_scores = [
            (agent, agent.get_fitness(self.market_data)) for agent in self.agents
        ]
        fitness_scores.sort(
        ...
        while len(new_agents) < self.population_size:
            # 10% chance to add completely random agent for diversity
            if np.random.random() < 0.1:
                # Create a completely new random agent
                offspring = self._create_random_agent()
            else:
    ...
    def _create_random_agent(self) -> "TradingAgent":
        """
        Create a completely random agent for diversity injection.
        ...
        random_agent = TradingAgent(
            agent_id=f"random_agent_{len(self.agents)}_{self.current_generation}",
            config=self.config,
            items_list=self.items_list,
            desired_item=desired_item,
            initial_inventory=None,  # Fresh random inventory
