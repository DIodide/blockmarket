    ...
    def _create_agents_from_config(
        ...
        for agent_config in agents_config:
            try:
                ...
                logger.debug(
                    f"Original inventory for {agent_config['agent_id']}: {original_inventory}"
                )
                ...
                logger.debug(
                    f"Filtered inventory for {agent_config['agent_id']}: {filtered_inventory}"
                )
                ...
                for trade in self.env.trade_history[-step_info["trades_executed"] :]:
                    ...
                    trade_formatted = {
                        ...
                        "requester_cell": getattr(
                            self._get_agent_by_id(trade.get("requester_id")),
                            "cell_key",
                            None,
                        ),
                        "target_cell": getattr(
                            self._get_agent_by_id(trade.get("target_id")),
                            "cell_key",
                            None,
                        ),
                    }
...
def main():
    ...
    socket_url = (
        sys.argv[1]
        if len(sys.argv) > 1
        else "https://geographical-clonic-jimena.ngrok-free.dev/model"
    )
