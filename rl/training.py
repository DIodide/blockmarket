...
def training_loop(
    ...
    last_summary_timestep = 0
    half_generation_interval = env.generation_length // 2
    ...
    try:
        initial_fitness = max(
            [agent.get_fitness(env.market_data) for agent in env.agents]
        )
    ...
    try:
        while running_flag["value"] and env.current_generation < max_generations:
            ...
            for timestep_in_gen in range(env.generation_length):
                ...
                current_state.update(
                    {
                        ...
                        "generation_progress": (timestep_in_gen + 1)
                        / env.generation_length,
                    }
                )

                # Generate LLM summary every half generation
                if (
                    env.current_timestep - last_summary_timestep
                ) >= half_generation_interval:
                    try:
                        logger.info(
                            f"Generating LLM summary at timestep {env.current_timestep}"
                        )

                        # Prepare data for LLM analysis
                        environment_data = current_state.copy()
                        agent_data = []
                            ...
                            agent_data.append(
                                {
                                    ...
                                    "fitness": agent.get_fitness(env.market_data),
                                    "successful_trades": agent.successful_trades,
                                    "attempted_trades": agent.attempted_trades,
                                }
                            )

                        trade_data = env.trade_history[-20:]  # Last 20 trades
                        generation_stats = {
                            "current_gen": env.current_generation,
                            "total_timesteps": env.current_timestep,
                            "generation_progress": (timestep_in_gen + 1)
                            / env.generation_length,
                        }

                        # Generate summary asynchronously to avoid blocking training
                        import threading
                        ...
                        def generate_summary_async():
                            ...
                            current_state["latest_summary"] = summary
                            current_state["summary_timestamp"] = time.time()

                        summary_thread = threading.Thread(target=generate_summary_async)
                        summary_thread.daemon = True
                        summary_thread.start()

                        last_summary_timestep = env.current_timestep
                ...
                if env.current_timestep % log_frequency == 0:
                    avg_reward = (
                        np.mean(generation_rewards[-log_frequency:])
                        if generation_rewards
                        else 0
                    )
            ...
            logger.info("=" * 60)
            logger.info(
                f"Generation {env.current_generation} completed in {generation_time:.2f}s"
            )
        ...
        logger.info(
            f"Best fitness achieved: {best_overall_fitness:.2f}"
            if best_overall_fitness != -float("inf")
            else "Best fitness achieved: No valid fitness recorded"
        )
        logger.info(
            f"Final average fitness: {np.mean([agent.get_fitness(env.market_data) for agent in env.agents]):.2f}"
        )
        logger.info("=" * 60)
...
def save_generation_models(env, model_save_path, generation, final=False):
    ...
    try:
        ...
        agents_by_fitness = sorted(
            env.agents, key=lambda a: a.get_fitness(env.market_data), reverse=True
        )

        # Save top 5 agents or all agents if final save
        num_to_save = (
            len(agents_by_fitness) if final else min(5, len(agents_by_fitness))
        )
        ...
        env_state_path = os.path.join(
            model_save_path, f"environment_gen{generation}.yaml"
        )
        if final:
