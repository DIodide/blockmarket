...
class UnifiedTradingApp:
            ...
            return jsonify(
                {
                    ...
                    "socket_enabled": self.config.get("socket", {}).get(
                        "enabled", False
                    ),
                }
            )
        ...
        def start_simulation(data):
            ...
            try:
                ...
                if self.sio and self.sio.connected:
                    self.sio.emit(
                        "simulation_error", {"error": str(e)}, namespace=namespace
                    )
    ...
    def _start_external_simulation(self, data: Dict[str, Any]):
        ...
        if self.external_simulation_running:
            logger.warning(
                "External simulation already running, stopping previous simulation"
            )
            self._stop_external_simulation()
        ...
        self.external_simulation_thread = threading.Thread(
            target=self._run_external_simulation, args=(external_env,), daemon=True
        )
    ...
    def _stop_external_simulation(self):
        ...
        if self.external_simulation_running:
            ...
            if (
                self.external_simulation_thread
                and self.external_simulation_thread.is_alive()
            ):
    ...
    def _run_external_simulation(self, external_env: TradingEnvironment):
        ...
        try:
            ...
            while self.external_simulation_running and self.running_flag["value"]:
                ...
                # Debug logging
                logger.debug(
                    f"Timestep {timestep}: trades_executed={step_info['trades_executed']}, total_trades_in_history={len(external_env.trade_history) if hasattr(external_env, 'trade_history') else 'No history'}"
                )
    ...
    def _emit_trade_data(
        self, external_env: TradingEnvironment, step_info: Dict[str, Any], timestep: int
    ):
        """
        ...
        try:
            ...
            # Debug logging
            logger.debug(
                f"_emit_trade_data called: timestep={timestep}, trades_executed={step_info['trades_executed']}"
            )
            logger.debug(
                f"Environment has trade_history: {hasattr(external_env, 'trade_history')}"
            )
            ...
            if (
                hasattr(external_env, "trade_history")
                and external_env.trade_history
                and step_info["trades_executed"] > 0
            ):
                # Get trades from this timestep
                recent_trades = []
                trades_to_process = external_env.trade_history[
                    -step_info["trades_executed"] :
                ]
                ...
                for trade in trades_to_process:
                    ...
                    trade_formatted = {
                        ...
                        "item_received": requester_received[0]
                        if requester_received
                        else None,
                        "amount_received": requester_received[1]
                        if requester_received
                        else None,
                        "requester_cell": getattr(
                            self._get_agent_by_id(
                                external_env, trade.get("requester_id")
                            ),
                            "cell_key",
                            None,
                        ),
                        "target_cell": getattr(
                            self._get_agent_by_id(external_env, trade.get("target_id")),
                            "cell_key",
                            None,
                        ),
                    }
            ...
            # Only emit trade data when there are actual trades
            if trade_data["trades_count"] > 0:
                if self.sio and self.sio.connected:
                    namespace = self.config.get("socket", {}).get("namespace", "/model")
                    logger.debug(f"Emitting to namespace {namespace}: {trade_data}")
                    self.sio.emit("trade", trade_data, namespace=namespace)
                    logger.info(
                        f"✅ Emitted trade data for timestep {timestep}: {len(trade_data['trades'])} trades"
                    )
                else:
                    logger.warning(
                        f"❌ Socket not connected, cannot emit trade data for timestep {timestep}"
                    )
            else:
                logger.debug(
                    f"No trades to emit for timestep {timestep} (trades_count = {trade_data['trades_count']})"
                )
    ...
    def _get_agent_by_id(
        self, env: TradingEnvironment, agent_id: str
    ...
    def _parse_bot_inventory_map(
        self, bot_inventory_map: Dict[str, Dict[str, int]]
    ) -> list:
    ...
    def _create_agents_from_config(
        self, agents_config: list, env: TradingEnvironment
    ) -> list:
            ...
            try:
                ...
                for item, amount in original_inventory.items():
                    if item in items_list:
                        ...
                        logger.warning(
                            f"Ignoring unknown item '{item}' for agent {agent_config['agent_id']}"
                        )
                ...
                # Debug logging
                logger.debug(
                    f"Created agent {agent.agent_id}: inventory={agent.inventory}, desired={agent.desired_item}"
                )
    ...
    def start_flask_server(self):
        ...
        logger.info(
            f"🌐 Flask web server started on {self.config['server']['host']}:{self.config['server']['port']}"
        )
        return True
        ...
        def training_wrapper():
            try:
                ...
                training_loop(
                    ...
                    model_save_frequency=self.config["training"][
    ...
    def run(self, enable_training=True, enable_flask=True, enable_socket=None):
        ...
        socket_enabled = (
            ...
            else self.config.get("socket", {}).get("enabled", False)
        )
        ...
        if enable_flask:
            logger.info(
                f"🌐 Web interface: http://localhost:{self.config['server']['port']}"
            )
        if socket_enabled:
            logger.info(
                f"🔌 Socket.IO client: {self.config.get('socket', {}).get('server_url', 'Not configured')}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Unified Multi-Agent Trading Application"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-training", action="store_true", help="Disable internal training"
    )
    parser.add_argument(
        "--no-socket", action="store_true", help="Disable socket client"
    )
